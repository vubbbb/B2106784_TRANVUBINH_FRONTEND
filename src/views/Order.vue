<template>
    <div class="q-pa-md">
        <q-table title="Đơn mượn" :rows="filteredOrders" :columns="columns" row-key="name" :filter="filter" grid
            hide-header>
            <template v-slot:item="props">
                <div class="q-pa-xs col-xs-12 col-sm-6 col-md-4 col-lg-3 grid-style-transition">
                    <q-card bordered flat>
                        <q-card-section>
                            <q-btn v-if="props.row.status === 'approved'" label="Đã trả sách" color="green"
                                @click="completedOrder(props.row._id)" />
                            <q-btn v-if="props.row.status === 'approved'" label="Từ chối" color="negative"
    ...
    setup() {
        ...
        onMounted(async () => {
            try {
                ...
                // filteredOrders.value = rows.value.sort((a, b) => b.index - a.index);
            } catch (error) {
                console.error(error)
            }
        })
